...
// CODE HERE
sayHelloButton.addEventListener("mouseover", problem2);
...
// CODE HERE
sayHelloButton.addEventListener("mouseout", problem3);
...
const repeatMyParam = () => {
    ...
    repeatEl.textContent = response.data;
...
// CODE HERE
// const queryRequest = () => {
//     axios.get('http://localhost:3000/query-test?myquery=a-really-awesome-query&anotherOne=DJ-Khalid')
...
const ohMy = () => {
    axios.get('http://localhost:3000/animals').then((res) => {
        const ohMyArr = res.data;

        for(let i=0; i<ohMyArr.length; i++){
            // console.log(ohMyArr[i])
            const body = document.querySelector('body')
            const p = document.createElement('p')
            p.textContent = ohMyArr[i];
            body.appendChild(p);
...
/*
    ...
    2: Send more than 1 query on the URL -- what happened? HOW TO ADD MORE THAN 1 QUERY IN THE SAME URL?? USING && ?
*/
